<template>
  <div>
    ...
    <el-dialog v-model="visible" :title="form.id ? '编辑品牌' : '新增品牌'" width="480px">
      <el-form label-width="88px">
        <el-form-item label="名称" required>
          <el-input v-model="form.name" maxlength="64" show-word-limit placeholder="品牌名称" />
        </el-form-item>
...
async function toggleStatus(row: BrandVO) {
...
async function onDelete(row: BrandVO) {
  await ElMessageBox.confirm(`确认删除品牌「${row.name}」？已被商品使用的品牌无法删除。`, "提示");
